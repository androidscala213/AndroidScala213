...
public abstract class ClassValue<T> {
    ...
    public T get(Class<?> type) {
        ...
        ClassValue.Entry<?>[] cache;
        ClassValue.Entry<T> e = probeHomeLocation(cache = getCacheCarefully(type), this);
        // racing e : current value <=> stale value from current cache or from stale cache
        // invariant:  e is null or an ClassValue.Entry with readable ClassValue.Entry.version and ClassValue.Entry.value
        if (match(e))
    ...
    private static ClassValue.Entry<?>[] getCacheCarefully(Class<?> type) {
        // racing type.classValueMap{.cacheArray} : null => new ClassValue.Entry[X] <=> new ClassValue.Entry[Y]
        ClassValueMap map = classValueMap.get(type);
        if (map == null)  return EMPTY_CACHE;
        ClassValue.Entry<?>[] cache = map.getCache();
        return cache;
        // invariant:  returned value is safe to dereference and check for an ClassValue.Entry
    }

    /** Initial, one-element, empty cache used by all Class instances.  Must never be filled. */
    private static final ClassValue.Entry<?>[] EMPTY_CACHE = { null };
     ...
     */
    private T getFromBackup(ClassValue.Entry<?>[] cache, Class<?> type) {
        ClassValue.Entry<T> e = probeBackupLocations(cache, this);
        if (e != null)
            return e.value();
    ...
    ClassValue.Entry<T> castEntry(ClassValue.Entry<?> e) { return (ClassValue.Entry<T>) e; }
    ...
    private T getFromHashMap(Class<?> type) {
        ...
        ClassValueMap map = getMap(type);
        for (;;) {
            ClassValue.Entry<T> e = map.startEntry(this);
            if (!e.isPromise())
                return e.value();
            try {
    ...
    boolean match(ClassValue.Entry<?> e) {
        ...
        // invariant:  If version matches, then e.value is readable (final set in ClassValue.Entry.<init>)
    }
    ...
    static class Entry<T> extends WeakReference<Version<T>> {
        final Object value;  // usually of type T, but sometimes (ClassValue.Entry)this
        ...
        Entry(Version<T> version) {
            super(version);
            this.value = this;  // for a promise, value is not of type T, but ClassValue.Entry!
        }
        ...
        ClassValue.Entry<T> refreshVersion(Version<T> v2) {
            assertNotPromise();
            @SuppressWarnings("unchecked")  // if !isPromise, type is T
                    ClassValue.Entry<T> e2 = new ClassValue.Entry<>(v2, (T) value);
            clear();
            // value = null -- caller must drop
            return e2;
        }
        static final ClassValue.Entry<?> DEAD_ENTRY = new ClassValue.Entry<>(null, null);
    }


    private static volatile HashMap<Class<?>, ClassValueMap> classValueMap = new HashMap<Class<?>, ClassValueMap>();
    ...
    private static ClassValueMap initializeMap(Class<?> type) {
        ...
        synchronized (CRITICAL_SECTION) {  // private object to avoid deadlocks
            ...
            if ((map = classValueMap.get(type)) == null)
                classValueMap.put(type, map = new ClassValueMap(type));
        }
        return map;
    }

    static <T> ClassValue.Entry<T> makeEntry(Version<T> explicitVersion, T value) {
        // Note that explicitVersion might be different from this.version.
        return new ClassValue.Entry<>(explicitVersion, value);

        // As soon as the ClassValue.Entry is put into the cache, the value will be
    ...
    static class ClassValueMap extends WeakHashMap<ClassValue.Identity, ClassValue.Entry<?>> {
        ...
        static ClassValue.Entry<?> loadFromCache(ClassValue.Entry<?>[] cache, int i) {
            ...
            // racing cache[i & (mask)] : null <=> ClassValue.Entry
